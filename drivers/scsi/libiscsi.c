...
#ifdef CONFIG_SCSI_ZADARA
#define __ISCSI_DBG_EH(_session, dbg_fmt, arg...)				\
	do {								\
		if (iscsi_dbg_lib_eh)					\
			iscsi_session_printk(KERN_INFO, _session,	\
					     "%s " dbg_fmt,		\
...
#define ISCSI_DBG_EH(_session, dbg_fmt, arg...)				\
	do {								\
		if (iscsi_dbg_lib_eh)					\
			iscsi_session_printk(KERN_INFO, _session,	\
					     "cmd %#02x,%ums: %s " dbg_fmt,		\
					     sc!=NULL ? sc->cmnd[0] : -1, sc!=0 ? jiffies_to_msecs(jiffies-sc->jiffies_at_alloc) : -1, __func__, ##arg);		\
	} while (0);
#else
#define ISCSI_DBG_EH(_session, dbg_fmt, arg...)				\
	do {								\
...
#ifdef CONFIG_SCSI_ZADARA
		__ISCSI_DBG_EH(session, "tmf timedout\n");
#else
...
#ifdef CONFIG_SCSI_ZADARA
	__ISCSI_DBG_EH(session, "tmf set timeout\n");
#else
