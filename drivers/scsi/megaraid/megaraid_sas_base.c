...
#ifdef CONFIG_MEGASAS_ZADARA
#include <linux/kallsyms.h>
#endif // CONFIG_MEGASAS_ZADARA
...
#ifdef CONFIG_MEGASAS_ZADARA
typedef void (*scsi_remove_device_fn_t)(struct scsi_device *sdev);

static scsi_remove_device_fn_t scsi_remove_device_fn_ptr;
#endif // CONFIG_MEGASAS_ZADARA
		...
		case MR_EVT_PD_REMOVED:
			...
			for (i = 0; i < MEGASAS_MAX_PD_CHANNELS; i++) {
				...
				if (instance->pd_list[pd_index].driveState
					...
					if (sdev1) {
#ifdef CONFIG_MEGASAS_ZADARA
						scsi_remove_device_fn_ptr(sdev1);
#else
...
#ifdef CONFIG_MEGASAS_ZADARA
        scsi_remove_device_fn_ptr = (scsi_remove_device_fn_t)kallsyms_lookup_name("__scsi_remove_device");
        if (!scsi_remove_device_fn_ptr) {
                printk(KERN_ERR "Failed to lookup __scsi_remove_device()");
                return -ENXIO;
        }
#endif // CONFIG_MEGASAS_ZADARA
