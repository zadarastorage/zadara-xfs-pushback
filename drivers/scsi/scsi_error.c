...
static int scsi_check_sense(struct scsi_cmnd *scmd)
{
	...
	case HARDWARE_ERROR:
		if (scmd->device->retry_hwerror)
			...
			set_host_byte(scmd, DID_TARGET_FAILURE);
#ifdef CONFIG_SCSI_ZADARA
		/* Patch for issue #7391:
		   if scmd->cmnd[0]==TEST_UNIT_READY && sshdr.asc==0x44, then return FAILED.
		   This will leave the command in the work_q, and scsi_eh_ready_devs will call scsi_eh_offline_sdevs that moves
		   device to the offline state. When device is offline, scsi_eh_flush_done_q doesn't retry the command, but finishes it.*/
		if (scmd->cmnd[0] == TEST_UNIT_READY && sshdr.asc == 0x44) {
			SCSI_LOG_ERROR_RECOVERY(1, scmd_printk(KERN_INFO, scmd, "%s: return FAILED\n", __func__));
			return FAILED;
		}
#endif
