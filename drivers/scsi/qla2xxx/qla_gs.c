...
{
	int ret, rval;
	fc_port_t fcport_local, *fcport = &fcport_local;
	...
	fcport->loop_id = vha->mgmt_svr_loop_id;
	fcport->d_id.b.domain = 0xff;
	fcport->d_id.b.area = 0xff;
	fcport->d_id.b.al_pa = 0xfa;
	rval = qla2x00_fabric_login(vha, fcport, &next_loopid, BIT_1);
	...
	if (rval != QLA_SUCCESS) {
		...
		/* fabric_login may have obtained a different loop id */
		vha->mgmt_svr_loop_id = fcport->loop_id;
		vha->flags.management_server_logged_in = 1;
		ql_dbg(ql_dbg_disc, vha, 0xffff,
			"Management server logged in: loopid=%x.\n",
