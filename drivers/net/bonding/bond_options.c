...
static int bond_option_send_udev_events_set(struct bonding *bond,
				  const struct bond_opt_value *newval);
...
#ifdef CONFIG_BONDING_ZADARA
static const struct bond_opt_value bond_send_udev_events_tbl[] = {
	{ "off", 0,  BOND_VALFLAG_DEFAULT},
	{ "on",  1,  0},
	{ NULL,  -1, 0}};
#endif /* CONFIG_BONDING_ZADARA */
...
#ifdef CONFIG_BONDING_ZADARA
	[BOND_OPT_SEND_UDEV_EVENTS] = {
		.id = BOND_OPT_SEND_UDEV_EVENTS,
		.name = "send_udev_events",
		.desc = "Send udev events on slaves mii changes",
		.values = bond_send_udev_events_tbl,
		.set = bond_option_send_udev_events_set
	},
#endif /* CONFIG_BONDING_ZADARA */
...
static int bond_option_send_udev_events_set(struct bonding *bond,
				       const struct bond_opt_value *newval)
{
	netdev_info(bond->dev, "Setting send_udev_events to %llu\n",
		    newval->value);
	bond->params.send_udev_events = newval->value;
