...
#define ZKLOG(fmt, ...) pr_info("%d:[%s:%u] "fmt"\n", current->pid, __FUNCTION__, __LINE__, ##__VA_ARGS__)
...
#ifdef CONFIG_BONDING_ZADARA
	/* 
     * after the bond is set up and some time passes,
     * we want to check (once) whether the bond is degraded.
     * this will tell us whether we have already checked this.
     */
	bool     after_boot_degraded_checked;
#endif /*CONFIG_BONDING_ZADARA*/
};
