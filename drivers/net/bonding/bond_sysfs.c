...
/* Show the send_udev_events flag. */
static ssize_t bonding_show_send_udev_events(struct device *d,
				    struct device_attribute *attr,
				    char *buf)
{
	struct bonding *bond = to_bond(d);

	return sprintf(buf, "%d\n", bond->params.send_udev_events);
}
static DEVICE_ATTR(send_udev_events, S_IRUGO | S_IWUSR,
		   bonding_show_send_udev_events, bonding_sysfs_store_option);
...
#ifdef CONFIG_BONDING_ZADARA
	&dev_attr_send_udev_events.attr,
#endif /* CONFIG_BONDING_ZADARA */
