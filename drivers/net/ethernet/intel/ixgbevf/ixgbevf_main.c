...
#ifdef CONFIG_IXGBEVF_ZADARA
	/* 
	ZADARA - Disable setting IXGBE_FLAG_IN_NETPOLL. It causes significant
	performance degradation	
	adapter->flags |= IXGBE_FLAG_IN_NETPOLL;
	*/
#endif /* CONFIG_IXGBEVF_ZADARA */
...
#ifdef CONFIG_IXGBEVF_ZADARA
	/*
	 * init_hw in some cases resets the MAC address,
	 * resulting in a non-functional VF.
	 */
	u8 mac_addr[6] = {0};
	dev_info(&adapter->pdev->dev, "[ixgbevf_reset] MAC address:%02x:%02x:%02x:%02x:%02x:%02x\n",
		     hw->mac.addr[0], hw->mac.addr[1], hw->mac.addr[2], hw->mac.addr[3], hw->mac.addr[4], hw->mac.addr[5]);
	memcpy(mac_addr, hw->mac.addr, sizeof(mac_addr));
#endif /*CONFIG_IXGBEVF_ZADARA*/
...
#ifdef CONFIG_IXGBEVF_ZADARA
	/* restore the address we had */
	if (!is_valid_ether_addr(adapter->hw.mac.addr)) {
		dev_info(&adapter->pdev->dev, "[ixgbevf_reset] after init_hw MAC address:%02x:%02x:%02x:%02x:%02x:%02x, restoring\n",
			     hw->mac.addr[0], hw->mac.addr[1], hw->mac.addr[2], hw->mac.addr[3], hw->mac.addr[4], hw->mac.addr[5]);
		memcpy(adapter->hw.mac.addr, mac_addr, sizeof(adapter->hw.mac.addr));
	}
#endif /*CONFIG_IXGBEVF_ZADARA*/
...
#ifdef CONFIG_IXGBEVF_ZADARA
	dev_info(&adapter->pdev->dev, "[ixgbevf_sw_init] MAC address:%02x:%02x:%02x:%02x:%02x:%02x\n",
		     hw->mac.addr[0], hw->mac.addr[1], hw->mac.addr[2], hw->mac.addr[3], hw->mac.addr[4], hw->mac.addr[5]);
#endif /*CONFIG_IXGBEVF_ZADARA*/
...
#ifdef CONFIG_IXGBEVF_ZADARA
	dev_info(&adapter->pdev->dev, "[ixgbevf_set_mac] MAC address:%02x:%02x:%02x:%02x:%02x:%02x\n",
		     hw->mac.addr[0], hw->mac.addr[1], hw->mac.addr[2], hw->mac.addr[3], hw->mac.addr[4], hw->mac.addr[5]);
#endif /*CONFIG_IXGBEVF_ZADARA*/
