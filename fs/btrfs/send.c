...
static int zsend_non_write_cmd(struct send_ctx *sctx);
#endif /*CONFIG_BTRFS_ZADARA*/
...
static int send_cmd(struct send_ctx *sctx)
{
	...
	u32 crc;
#ifdef CONFIG_BTRFS_ZADARA
	if (sctx->send_filp == NULL)
		return zsend_non_write_cmd(sctx);
#endif /*CONFIG_BTRFS_ZADARA*/
...
#ifdef CONFIG_BTRFS_ZADARA
			if (sctx->send_filp == NULL) {
				ZBTRFSLOG_TAG(sctx->send_root->fs_info, Z_KERR, ZKLOG_TAG_SR,
					          "send_root[%llu] parent_root[%llu] ino(%llu) gen %llu=>%llu NEW GEN should not happen for block-virt!!!",
					          sctx->send_root->objectid, sctx->parent_root->objectid, sctx->cur_ino,
					          right_gen, left_gen);
				ret = -EPROTO;
				goto out;
			}
...
#ifdef CONFIG_BTRFS_ZADARA
static int full_send_tree(struct btrfs_compare_trees_checkpoint *tree_cp,
				btrfs_changed_cb_t changed_cb_func, 
				struct send_ctx *sctx)
...
#endif /*CONFIG_BTRFS_ZADARA*/
{
...
#ifdef CONFIG_BTRFS_ZADARA
	if (tree_cp == NULL) {
	ZBTRFSLOG_TAG(send_root->fs_info, Z_KDEB1, ZKLOG_TAG_CHCKP, "Full send L[%llu] - no cp", send_root->objectid);
#endif /*CONFIG_BTRFS_ZADARA*/
	...
	ZBTRFSLOG_TAG(send_root->fs_info, Z_KDEB1, ZKLOG_TAG_CHCKP, "Full send L[%llu]: rearm(%llu,%s,%llu)", send_root->objectid, key.objectid, btrfs_fs_key_type_to_str(key.type), key.offset);
	}
#endif /*CONFIG_BTRFS_ZADARA*/
...
#ifdef CONFIG_BTRFS_ZADARA
		ret = changed_cb_func(send_root, NULL/*right_root*/, path/*left_path*/, NULL/*right_path*/,
...
#ifdef CONFIG_BTRFS_ZADARA
		ret = full_send_tree(NULL/*tree_cp*/, changed_cb, sctx);
#else /*CONFIG_BTRFS_ZADARA*/
