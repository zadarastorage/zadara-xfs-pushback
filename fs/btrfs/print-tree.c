...
void btrfs_print_leaf(struct btrfs_root *root, struct extent_buffer *l)
{
	...
	for (i = 0 ; i < nr ; i++) {
		...
		case BTRFS_EXTENT_DATA_KEY:
			...
			if (btrfs_is_bv_file_extent_item(l, i)) {
				struct btrfs_bv_file_extent_item *bfi = btrfs_item_ptr(l, i, struct btrfs_bv_file_extent_item);
				printk(KERN_INFO "\t\tblock-virt disk bytenr %llu\n", btrfs_bv_file_extent_disk_bytenr(l, bfi));
				break;
			}
